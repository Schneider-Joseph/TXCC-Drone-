...
print(f"Number of images loaded: {len(images_to_stitch)}")

# Display or save the result
if stitched_image is not None:
    # Resize for display on a small screen if needed
    stitched_image = imutils.resize(stitched_image, width=800) 
    cv2.imwrite('stitched_output.jpg', stitched_image)
    cv2.imwrite("panorama.jpg", stitched_image) # Save the image
